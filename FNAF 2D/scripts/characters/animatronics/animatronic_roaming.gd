...
@export var cam_path : Array[String]
...
var current_cam_index_property:
	...
	set(value):
		...
		var cam_name : String = cam_path[current_cam_index_property] if current_cam_index_property >= 0 else ""
...
func move() -> void:
	...
	if move_forward:
		current_cam_index_property += 1 if current_cam_index_property < cam_path.size() - 1 else -current_cam_index_property - 1
	else:
